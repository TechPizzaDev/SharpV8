...
:EnsureLogsDir
if not exist logs\ goto CreateLogsDir
...
call gclient config https://chromium.googlesource.com/v8/v8 >..\logs\config.log 2>&1
if errorlevel 1 goto Error
call gclient sync --no-history --shallow -r %v8commit% >..\logs\sync.log 2>&1
if errorlevel 1 goto Error
:SyncClientDone
...
if errorlevel 1 goto Error
call git apply --reject --ignore-whitespace ..\..\V8Patch.diff 2>..\..\logs\applyV8Patch.log
if errorlevel 1 goto Error
cd ..
:ApplyPatchesDone
...
call git clone -n https://github.com/nlohmann/json.git 2>..\logs\cloneJson.log
if errorlevel 1 goto Error
cd json
call git checkout %jsontag% 2>..\..\logs\checkout.log
if errorlevel 1 goto Error
cd ..
:DownloadMiscDone
...
cd v8
call git diff --ignore-space-change --ignore-space-at-eol >V8Patch.diff 2>..\..\logs\createV8Patch.log
if errorlevel 1 goto Error
cd ..
:CreatePatchesDone
...
copy build\v8\V8Patch.diff .\ >nul
if errorlevel 1 goto Error
:ImportPatchesDone
